...
public class FirebaseAuthHelper {
    ...
    public static func AsyncSimpleAuthedRequest(
        endpoint: String, method: HTTPMethod, params: [String: String]? = nil
    ) async -> GenericResponse {
        await withCheckedContinuation { continuation in

            FirebaseAuthHelper.SimpleAuthedRequest(
    ...
    public static func SimpleAuthedRequest(
        endpoint: String, method: HTTPMethod, params: [String: String]? = nil,
        completion: @Sendable @escaping (GenericResponse) -> Void
    ) {
        if let user = Auth.auth().currentUser {
            user.getIDToken { token, error in
                if let error = error {
                    return completion(
                        GenericResponse(
                            status: .ERROR, code: 0, message: error.localizedDescription))
                }
                guard let token = token else {
                    return completion(
                        GenericResponse(status: .ERROR, code: 401, message: "Failed to get token"))
                }
                ...
                return FirebaseAuthHelper.SimpleRequest(
                    endpoint: url, method: method, token: token, completion: completion)
            }
        }
    }

    public static func AsyncAuthedBodyRequest<T: Codable>(
        endpoint: String, method: HTTPMethod, data: T, contentType: ContentType
    ) async -> GenericResponse {
        await withCheckedContinuation { continuation in
            FirebaseAuthHelper.AuthedBodyRequest(
                endpoint: endpoint, method: method, data: data, contentType: contentType
            ) { resp in
                continuation.resume(returning: resp)
            }
        }
    ...
    public static func AuthedBodyRequest<T: Codable>(
        endpoint: String, method: HTTPMethod, data: T, contentType: ContentType,
        completion: @Sendable @escaping (GenericResponse) -> Void
    ) {
        if let user = Auth.auth().currentUser {
            user.getIDToken { token, error in
                ...
                return FirebaseAuthHelper.BodyRequest(
                    endpoint: endpoint, data: data, token: token, contentType: contentType,
                    method: method, completion: completion)
            }
        }
    }

    public static func AsyncSimpleRequest(
        endpoint: String, method: HTTPMethod, params: [String: String]? = nil
    ) async -> GenericResponse {
        await withCheckedContinuation { continuation in

            FirebaseAuthHelper.SimpleRequest(endpoint: endpoint, method: method) { resp in
                continuation.resume(returning: resp)
            }
        }
    }

    public static func SimpleRequest(
        endpoint: String, method: HTTPMethod, user: String? = nil, password: String? = nil,
        token: String? = nil, completion: @Sendable @escaping (GenericResponse) -> Void
    ) {
        ...
        AF.request(url, method: method, headers: headers).validate().response { response in
            ...
            case .success:
                completion(
                    GenericResponse(
                        status: .OK, code: response.response?.statusCode ?? 0,
                        message: "Request succeeded", data: response.data))
                        ...
                        completion(
                            GenericResponse(
                                ...
                                message: String(data: data, encoding: .utf8)
                                    ?? "Requested failed with code: \(httpResponse.statusCode)"))
                    } else {
                        completion(
                            GenericResponse(
                                status: .ERROR, code: httpResponse.statusCode,
                                message: "Request failed with code: \(httpResponse.statusCode)"))
    ...
    public static func AsyncBodyRequest<T: Codable>(
        endpoint: String, data: T, user: String? = nil, password: String? = nil,
        token: String? = nil, contentType: ContentType, method: HTTPMethod
    ) async -> GenericResponse {
        await withCheckedContinuation { continuation in
            FirebaseAuthHelper.BodyRequest(
                endpoint: endpoint, data: data, user: user, password: password, token: token,
    ...
    public static func BodyRequest<T: Codable>(
        endpoint: String, data: T, user: String? = nil, password: String? = nil,
        token: String? = nil, contentType: ContentType, method: HTTPMethod,
        completion: @Sendable @escaping (GenericResponse) -> Void
    ) {
        ...
        case .json:
            ...
            guard let jsonData = try? JSONEncoder().encode(data),
                ...
                completion(
                    GenericResponse(status: .ERROR, code: 0, message: "Failed to encode JSON"))
                return
            }
            body = jsonDict
        ...
        case .form:
            ...
            guard let dict = try? FirebaseAuthHelper.convertToDictionary(data) else {
                completion(
                    GenericResponse(status: .ERROR, code: 0, message: "Failed to encode form data"))
                return
            }
            body = dict
            ...
            .validate().response { response in
                ...
                case .success:
                    if let resp = response.response {
                        completion(
                            GenericResponse(
                                status: .OK, code: resp.statusCode, message: "Request succeeded",
                                data: response.data))
                    }
